...
std::vector<double> COSMethod::recoverCDF(double a, double b, size_t N, const std::function<std::complex<double>(double)>& chf, const std::vector<double>& x)
{
    /**
...
std::vector<double> COSMethod::recoverPDF(double a, double b, size_t N, const std::function<std::complex<double>(double)>& chf, const std::vector<double>& x)
{
    /**
...
std::pair<double, size_t> COSMethod::invertCDF(double a, double b, size_t N, const std::function<std::complex<double>(double)>& chf, double p, size_t max_iter, double tol)
{
    /**
...
COSMethod::Coefficients COSMethod::precomputeCoefficients(double a, double b, size_t N, const std::function<std::complex<double>(double)>& chf)
{
    /**
...
double COSMethod::evaluateCDF(const Coefficients& coeffs, double x)
{
    /**
...
double COSMethod::evaluatePDF(const Coefficients& coeffs, double x)
{
    /**
...
std::pair<double, size_t> COSMethod::invertCDF_Optimized(double a, double b, size_t N, const std::function<std::complex<double>(double)>& chf, double p, size_t max_iter, double tol)
{
    /**
...
double CIRSampler::sampleCIR(double kappa, double vbar, double sigma,
                             double v_s, double s, double t,
                             std::default_random_engine& randomEngine)
{
    /**
     * Exact CIR Sampling via Noncentral Chi-Squared Distribution (Boost)
     * 
     * V(t) | V(s) ~ (1/2c) · χ²(δ, λ)
     * 
     * where:
     ...
     *   τ = t - s
     * 
     * Uses Boost's noncentral chi-squared distribution for exact sampling.
     * 
    ...
    // Generate uniform and invert CDF
    std::uniform_real_distribution<double> uniform(0.0, 1.0);
    double u = uniform(randomEngine);
    
    // Inverse CDF to get sample
...
std::complex<double> ChFIntegratedVariance::modifiedBesselI(double nu, std::complex<double> z)
{
    ...
    if (std::abs(z) > 30.0) {
        return std::exp(z) / std::sqrt(2.0 * M_PI * z);
    }
