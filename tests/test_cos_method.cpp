...
TEST_F(COSMethodTest, Comparenewtons) {
    ...
    for (size_t N : N_values) {
        ...
        for (double p : test_probs) {
            auto [quantile, numIters] = COSMethod::invertCDF(a, b, N,  Utils::stdNormChF, p);
    ...
    for (size_t N : N_values) {
        ...
        for (double p : test_probs) {
            auto [quantile, numIters] = COSMethod::invertCDF_Optimized(a, b, N,  Utils::stdNormChF, p);
            (void)quantile;  // Suppress unused warning
            total_numIters += numIters;
        }
...
TEST_F(COSMethodTest, AccuracyVsN) {
    ...
    for (size_t N : N_values) {
        ...
        for (int run = 0; run < num_runs; ++run) {
            pdf_cos_values = COSMethod::recoverPDF(a, b, N,  Utils::stdNormChF, test_points);
        }
...
TEST_F(COSMethodTest, AccuracyVsN_faster) {
    ...
    for (size_t N : N_values) {
        ...
        auto coeffs = COSMethod::precomputeCoefficients(a, b, N, Utils::stdNormChF);
        ...
        for (int run = 0; run < num_runs; ++run) {
            ...
            for (size_t i = 0; i < test_points.size(); ++i) {
                pdf_cos_values[i] = COSMethod::evaluatePDF(coeffs, test_points[i]);
            }
        }
