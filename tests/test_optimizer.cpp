...
TEST(OptimizerTest, LM_UnderdeterminedCircle) {
    ...
    EXPECT_TRUE(result.converged);
    double r2 = result.params[0] * result.params[0] + result.params[1] * result.params[1];
...
TEST(OptimizerTest, LM_BoundedOptimization) {
    ...
    // reaches the constrained optimum even though convergence flag won't trigger
...
TEST(HimmelblauTest, ConvergenceComparison) {
    ...
    for (auto &x0 : starts) {
        ...
        EXPECT_TRUE(gdR.converged) << "GD failed from (" << x0[0] << "," << x0[1] << ")";
        EXPECT_TRUE(lbR.converged) << "LBFGS failed from (" << x0[0] << "," << x0[1] << ")";
        EXPECT_TRUE(lmR.converged) << "LM failed from (" << x0[0] << "," << x0[1] << ")";
        ...
        std::cout << "(" << std::setw(5) << x0[0] << "," << std::setw(5) << x0[1] << ")"
                  << " | " << identifyMin(gdR.params) << " " << std::setw(5) << gdR.iterations
                  << "it"
                  << " | " << identifyMin(lbR.params) << " " << std::setw(5) << lbR.iterations
                  << "it"
                  << " | " << identifyMin(lmR.params) << " " << std::setw(5) << lmR.iterations
                  << "it"
                  << "\n";
    }
    std::cout << std::string(90, '-') << "\n\n";
