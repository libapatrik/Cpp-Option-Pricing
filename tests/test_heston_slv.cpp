...
// ============================================================================
// TEST ORDERING (Most Important → Basic Correctness)
// ============================================================================
//
// TIER 1: ESSENTIAL VALIDATION (prove the implementation works)
...
//
// TIER 2: DIAGNOSTIC INSIGHT (understand how/why it works)
...
//
// TIER 3: IMPLEMENTATION COMPARISON (compare approaches)
...
//
// TIER 4: BASIC UNIT TESTS (correctness checks)
...
protected:
    ...
    double r = 0.0;
    double v0 = 0.04;
    double kappa = 2.0;        // Increased from 0.5 for stronger mean reversion
    double vbar = 0.04;
    double sigma_v = 0.3;      // Reduced from 1.0 to satisfy Feller: 0.3^2=0.09 < 0.16
    double rho = -0.7;         // Reduced from -0.9 for more stability
...
TEST_F(StoepComparisonTest, PureQEHestonPricingErrors)
{
    ...
    for (double T : {0.5, 1.0}) {
        ...
        for (size_t i = 0; i <= nSteps; ++i) times[i] = i * T / nSteps;
        ...
        for (double K : {0.90, 0.95, 1.0, 1.05, 1.10}) {
            ...
            for (double s : terminalSpots) payoffSum += std::max(s - K, 0.0);
...
// This is the primary validation that we match paper results.
// ----------------------------------------------------------------------------
TEST_F(StoepComparisonTest, VanDerStoepParameters) {
    ...
    std::cout << "  K\\T   ";
    for (double t : T) std::cout << std::setw(8) << t;
    std::cout << "\n";
    for (size_t k = 0; k < K.size(); ++k) {
    ...
    // Our QE scheme with Feller violation (σ²=0.90 > 2κv̄=0.18) has ~300bp error
...
TEST_F(StoepComparisonTest, HestonConsistentSurface_COS) {
    ...
    for (double t : T) std::cout << std::setw(8) << t;
    std::cout << "\n  ------";
    for (size_t i = 0; i < T.size(); ++i) std::cout << "--------";
    std::cout << "\n";
    for (size_t k = 0; k < K.size(); ++k) {
        std::cout << "  " << std::fixed << std::setprecision(2) << K[k] << "  ";
        for (size_t t = 0; t < T.size(); ++t) {
...
TEST_F(StoepComparisonTest, EVgivenSAnalysis)
{
    ...
    spotQ1 /= q1end; varQ1 /= q1end;
    spotQ4 /= (nPaths - q4start); varQ4 /= (nPaths - q4start);
...
TEST_F(StoepComparisonTest, DupireComponentDiagnostic)
{
    ...
    auto volFunc_K = [&surface, time](double k) { return surface.impliedVolatility(k, time); };
    auto volFunc_T = [&surface, strike](double t) { return surface.impliedVolatility(strike, t); };
...
TEST_F(StoepComparisonTest, OnTheFlyVsGridPricing)
{
    ...
    std::vector<double> times(nSteps + 1);
    for (size_t i = 0; i <= nSteps; ++i) times[i] = i * testT / nSteps;
    ...
    for (double K : testK) {
        ...
        double payoffSum = 0.0;
        for (const auto& [s, v] : terminalsOTF) payoffSum += std::max(s - K, 0.0);
    ...
    for (double K : testK) {
        ...
        double payoffSum = 0.0;
        for (const auto& [s, v] : terminalsGrid) payoffSum += std::max(s - K, 0.0);
        double slvPrice = discount.discount(testT) * payoffSum / nPaths;
        double slvIV = COSPricer::impliedVol(slvPrice, S0, K, r, testT, true);
