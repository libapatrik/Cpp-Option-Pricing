...
//   3. VanDerStoepParameters       - Paper validation with exact parameters
//   (~8bp)
...
{
protected:
	// Modified parameters that SATISFY Feller condition: 2*kappa*vbar >=
	// sigma_v^2 With kappa=2.0, vbar=0.04: 2*2*0.04 = 0.16 >= sigma_v^2 So
	// sigma_v <= 0.4 satisfies Feller
	double S0 = 1.0;
	double r = 0.0;
	double v0 = 0.04;
	double kappa = 2.0;
	...
	generateHestonIVs_COS(const std::vector<double> &strikes,
						  const std::vector<double> &maturities)
	{
		...
		for (size_t t_idx = 0; t_idx < maturities.size(); ++t_idx)
		{
			...
			auto chf = [&hestonCF](double u) { return hestonCF(u); };
	...
	std::unique_ptr<VolatilitySurface> createVolSurface()
	{
		...
		for (size_t t = 0; t < T.size(); ++t)
		{
			...
			for (size_t k = 0; k < K.size(); ++k)
			{
				...
				vols[t][k] =
					atmVol - 0.15 * logK + 0.02 / std::sqrt(T[t]) * logK * logK;
	...
	double priceCallMC(const std::vector<std::pair<double, double>> &terminals,
					   double K, double T)
	{
		double sum = 0.0;
	...
	double priceCallHeston(double K, double T, const HestonModel &model,
						   size_t nPaths)
	{
...
TEST_F(StoepComparisonTest, DupireFlatVolDiagnostic)
{
	...
	std::cout << "For flat vol surface, local vol should equal implied vol "
				 "exactly.\n\n";
	...
	std::vector<std::vector<double>> flatVols(
		T.size(), std::vector<double>(K.size(), flatVol));
	...
	for (double spot : testSpots)
	{
		for (double time : testTimes)
		{
			...
			std::cout << "  " << std::setw(6) << spot << "    " << std::setw(4)
					  << time << "    " << std::setw(6) << localVol * 100 << "%"
					  << "    " << std::showpos << std::setw(6) << error_bp
					  << std::noshowpos << "\n";
		}
	}
	...
	EXPECT_LT(maxError, 10.0)
		<< "Dupire formula has significant error on flat vol surface!";
}
...
TEST_F(StoepComparisonTest, PureQEHestonPricingErrors)
{
	...
	std::cout
		<< "Baseline: how accurate is QE scheme for van der Stoep params?\n\n";
	...
	for (double T : {0.5, 1.0})
	{
		...
		for (double K : {0.90, 0.95, 1.0, 1.05, 1.10})
		{
			...
			std::cout << std::fixed << std::setprecision(2) << "  " << T
					  << "    " << K << "    " << std::setprecision(6)
					  << cosPrice << "    " << qePrice << "  "
					  << std::setprecision(1) << std::showpos << errorBP
					  << std::noshowpos << "\n";
		}
		std::cout << "\n";
	}
	std::cout << "Max QE error: " << std::setprecision(1) << maxError
			  << " bp\n";
	std::cout
		<< "(This is the baseline - SLV cannot do better than pure Heston)\n";
}
...
TEST_F(StoepComparisonTest, VanDerStoepParameters)
{
	...
	std::cout << (feller >= sigSq ? " (satisfied)"
	...
	std::cout << std::setw(6) << "T" << std::setw(8) << "K" << std::setw(10)
			  << "Market" << std::setw(10) << "SLV" << std::setw(10) << "Error"
			  << "\n";
	...
	for (double maturity : testT)
	{
		...
		std::cout << "  [T=" << maturity << "] Calibration: " << calibIters
				  << " iters\n";
		...
		for (double strike : testK)
		{
			...
			std::cout << std::setw(6) << maturity << std::setw(8) << strike
					  << std::setw(9) << mktVol * 100 << "%" << std::setw(9)
					  << slvVol * 100 << "%" << std::setw(8) << std::showpos
					  << error_bp << "bp" << std::noshowpos << "\n";
		}
		std::cout << "\n";
	}

	std::cout << "Max absolute error: " << maxAbsError << " bp\n";
	std::cout << "(Paper uses low-bias scheme; our QE scheme has higher error "
				 "with Feller violation)\n";

	// Paper achieves 10-30bp using Broadie-Kaya (exact) or low-bias schemes.
	// Our QE scheme with Feller violation (σ^2=0.90 > 2κv̄=0.18) has ~300bp
	// error due to variance hitting zero. This is a known limitation For
	// Feller-satisfying params (like HestonConsistentSurface_COS), we achieve
	// <100bp.
	EXPECT_LT(maxAbsError, 350.0)
		<< "Errors exceed expected range for Feller-violating params!";
}
...
TEST_F(StoepComparisonTest, HestonConsistentSurface_COS)
{
	...
	std::cout
		<< "  L(S,t) should be ~1 everywhere -> errors = pure SLV MC noise\n";
	std::cout << "============================================================="
				 "===\n\n";
	...
	for (double maturity : testT)
	{
		...
		for (double strike : testK)
		{
			...
			double slvPrice = discountCurve.discount(maturity) * sumPayoff /
							  slvTerminals.size();
			double slvVol = BlackScholesFormulas::impliedVolatility(
	...
	std::cout << "Max absolute error: " << maxAbsError << " bp\n";
	std::cout << "(Errors include: SLV MC noise + Dupire numerical derivatives "
...
TEST_F(StoepComparisonTest, EVgivenSAnalysis)
{
	...
	std::cout << "Bin  Spot_lo  Spot_mid  Spot_hi     E[V|S]    sqrt(E[V|S])   "
				 "E[S]    σ_S\n";
	std::cout << "-------------------------------------------------------------"
				 "-------------\n";
	...
	for (size_t b = 0; b < nBins; ++b)
	{
		...
		std::cout << std::setw(3) << b << "  " << std::setprecision(4)
				  << std::setw(7) << minSpot << "  " << std::setw(8) << meanSpot
				  << "  " << std::setw(7) << maxSpot << "  "
				  << std::setprecision(6) << std::setw(10) << meanVar << "  "
				  << std::setprecision(2) << std::setw(12)
				  << std::sqrt(meanVar) * 100 << "%"
	...
	std::cout << "\nUnconditional stats:\n";
	std::cout << "  E[S_T] = " << std::setprecision(4) << meanS
			  << " (forward = " << S0 * std::exp(r * T) << ")\n";
	std::cout << "  E[V_T] = " << std::setprecision(6) << meanV << " (v0=" << v0
			  << ", vbar=" << vbar << ")\n";
	std::cout << "  √E[V_T] = " << std::setprecision(2)
			  << std::sqrt(meanV) * 100 << "%\n";
...
TEST_F(StoepComparisonTest, DupireComponentDiagnostic)
{
	...
	double dSigma_dK =
		(volFunc_K(strike + h_K) - volFunc_K(strike - h_K)) / (2 * h_K);
	double dSigma_dT =
		(volFunc_T(time + h_T) - volFunc_T(time - h_T)) / (2 * h_T);
	double d2Sigma_dK2 = (volFunc_K(strike + h_K) - 2 * volFunc_K(strike) +
						  volFunc_K(strike - h_K)) /
						 (h_K * h_K);

	// Black-Scholes d1, d2
	double volSqrtT = impliedVol * std::sqrt(time);
	double d1 =
		(std::log(spot / strike) + (r + 0.5 * impliedVol * impliedVol) * time) /
		volSqrtT;
	double d2 = d1 - volSqrtT;

	// Dupire formula components
	double numerator =
		1.0 + (2.0 * time / impliedVol) * (dSigma_dT + r * strike * dSigma_dK);
	...
	std::cout << "Numerical derivatives (step sizes: h_K=" << h_K
			  << ", h_T=" << h_T << "):\n";
	std::cout << "  dσ/dK:          " << std::setw(12) << dSigma_dK
			  << " (negative = downward skew)\n";
	std::cout << "  dσ/dT:          " << std::setw(12) << dSigma_dT
			  << " (negative = term structure flattening)\n";
	std::cout << "  d²σ/dK²:        " << std::setw(12) << d2Sigma_dK2
			  << " (positive = smile convexity)\n\n";
	...
	std::cout << "  σ²:             " << std::setw(12)
			  << impliedVol * impliedVol << "\n";
	std::cout << "  Numerator:      " << std::setw(12) << numerator << "\n";
	std::cout << "  Denominator:    " << std::setw(12) << denominator << "\n";
	std::cout << "  Ratio (N/D):    " << std::setw(12)
			  << numerator / denominator << "\n\n";

	std::cout << "Denominator components:\n";
	std::cout << "  1 (constant):        " << std::setw(10) << 1.0 << "\n";
	std::cout << "  2*d1*K*dσ/dK*√T:     " << std::setw(10)
			  << 2.0 * d1 * K_dSigma_dK_sqrtT << "\n";
	std::cout << "  d1*d2*(K*dσ/dK*√T)²: " << std::setw(10)
			  << d1 * d2 * K_dSigma_dK_sqrtT * K_dSigma_dK_sqrtT << "\n";
	std::cout << "  K*d²σ/dK²*√T*K*σ*√T: " << std::setw(10)
			  << K_d2Sigma_dK2_sqrtT * K_sigma_sqrtT << "\n\n";

	std::cout << "Result:\n";
	std::cout << "  Local vol:      " << localVol * 100 << "%\n";
	std::cout << "  Ratio LV/√v0:   " << localVol / sqrtV0
			  << " (should be ~1.0)\n";
...
{
	/**
	 * Compare analytical Dupire (HestonLocalVol) to interpolated IV surface
	 * Dupire. The analytical version should be more accurate (closer to √v0
	 * ratio).
	 */
	...
	FlatDiscountCurve discountCurve(r);
	VolatilitySurface interpolatedSurface(strikes, maturities, ivGrid,
										  discountCurve);
	...
	for (double k : {0.90, 0.95, 1.00, 1.05, 1.10})
	{
		...
		maxRatioError_analytical = std::max(
			maxRatioError_analytical, std::abs(lv_analytical / sqrtV0 - 1.0));
		maxRatioError_interpolated =
			std::max(maxRatioError_interpolated,
	...
	std::cout << "  Analytical:    " << std::setprecision(2)
			  << maxRatioError_analytical * 100 << "%\n";
	std::cout << "  Interpolated:  " << std::setprecision(2)
			  << maxRatioError_interpolated * 100 << "%\n";

	// The analytical Dupire should have much smaller ratio errors than
	// interpolated This demonstrates why we need COS-based Dupire for accurate
	// SLV calibration
...
TEST_F(StoepComparisonTest, OnTheFlyVsGridPricing)
{
	...
	for (double K : testK)
	{
		...
		std::cout << "  " << std::fixed << std::setprecision(2) << K << "   "
				  << cosIV * 100 << "%"
				  << "   " << slvIV * 100 << "%"
				  << "  " << std::setprecision(1) << std::showpos << errBp
				  << std::noshowpos << "\n";
	}
	std::cout << "Max error (on-the-fly): " << std::setprecision(1)
			  << maxErrorOTF << " bp\n\n";
	...
	std::cout << "Max error (calibrated): " << std::setprecision(1)
			  << maxErrorGrid << " bp\n\n";

	std::cout << "Comparison: On-the-fly = " << maxErrorOTF
			  << " bp, Grid = " << maxErrorGrid << " bp\n";
}
...
TEST_F(StoepComparisonTest, LeverageGridVsOnTheFly)
{
	...
	// First, get the calibrated leverage at several spots for time index 8
	// (midpoint of T=0.5)
	size_t midTimeIdx = nSteps / 2; // approx t = 0.25
	double midTime = times[midTimeIdx];

	std::cout << "Comparing leverage at t=" << midTime << " (time index "
			  << midTimeIdx << "):\n";
	...
	for (size_t pct : percentiles)
	{
		...
		size_t s_idx = std::min(static_cast<size_t>(frac * (nSpotGrid - 1)),
								nSpotGrid - 1);
...
{
	/**
	 * For a Heston-consistent surface, the local vol at ATM should be close to
	 * √v0. Test that LV(S0, T) / √v0 ≈ 1.0 for various maturities.
	 */
	...
	for (double T : maturities)
	{
		...
		// Local vol ratio should be within 15% of 1.0 for Feller-satisfying
		// params
...
TEST_F(HestonLocalVolTest, SkewPattern)
{
	 ...
	 * This reflects the leverage effect: down moves correlate with vol
	 * increases.
	 */
	HestonLocalVol hestonLV(kappa, vbar, sigma_v, rho, v0, r, S0);
	double T = 0.5;
	...
	// With rho < 0, OTM put vol > ATM vol > OTM call vol (downward sloping
	// skew)
	EXPECT_GT(lv_otm_put, lv_atm * 0.95) << "OTM put vol should be >= ATM vol";
	EXPECT_LT(lv_otm_call, lv_atm * 1.10)
		<< "OTM call vol should be <= ATM vol";
}
...
TEST_F(HestonLocalVolTest, VanDerStoepParameters)
{
	...
	HestonLocalVol stoepLV(stoep_kappa, stoep_vbar, stoep_sigma_v, stoep_rho,
						   stoep_v0, r, S0);
	...
	std::cout << "Feller condition: 2κθ = " << 2 * stoep_kappa * stoep_vbar
			  << " vs σ² = " << stoep_sigma_v * stoep_sigma_v
			  << " (VIOLATED)\n\n";
	...
	for (double T : {0.5, 1.0})
	{
		for (double k : {0.90, 0.95, 1.00, 1.05, 1.10})
		{
			...
			std::cout << std::fixed << std::setprecision(2) << T << "    " << k
					  << "    " << lv * 100 << "%"
					  << "    " << std::setprecision(3) << lv / sqrtV0 << "\n";
		}
		std::cout << "\n";
	...
	// Even for Feller-violating params, the analytical Dupire should give
	// reasonable values
...
TEST_F(HestonLocalVolTest, DerivativeAccuracy)
{
	...
	for (size_t i = 0; i < nSteps; ++i)
	{
		...
		double h = K * 0.005;
		double C_plus =
			COSPricer::callPrice(S0, K + h, r, T, cf, 256, 12.0, std::sqrt(v0));
		double C_0 =
			COSPricer::callPrice(S0, K, r, T, cf, 256, 12.0, std::sqrt(v0));
		double C_minus =
			COSPricer::callPrice(S0, K - h, r, T, cf, 256, 12.0, std::sqrt(v0));
	...
	std::cout << "Integral of e^{rT}·d²C/dK² from K=" << K_min << " to "
			  << K_max << ": " << integral << "\n";
	std::cout << "(Should be close to 1.0 minus tail mass)\n";

	// The integral won't be exactly 1.0 because we're not integrating over all
	// strikes but it should be reasonably close (0.8-1.0 for the range
	// [0.5S0, 1.5S0])
	EXPECT_GT(integral, 0.7)
		<< "Density integral should capture most of the mass";
...
TEST_F(COSPricerTest, BlackScholesATMCall)
{
	...
	double bsPrice = S0 * Utils::stdNormCdf(d1) -
					 K * std::exp(-r * T) * Utils::stdNormCdf(d2);

	EXPECT_NEAR(cosPrice, bsPrice, 1e-6)
		<< "COS should match BS analytical for ATM call";
}
...
TEST_F(COSPricerTest, BlackScholesOTMCall)
{
	...
	EXPECT_NEAR(cosPrice, bsPrice, 1e-5)
		<< "COS should match BS analytical for OTM call";
}
...
TEST_F(COSPricerTest, BlackScholesPut)
{
	...
	double d1 = (std::log(S0 / K) + (r + 0.5 * sigma * sigma) * T) /
				(sigma * std::sqrt(T));
	double d2 = d1 - sigma * std::sqrt(T);
	double bsPrice = K * std::exp(-r * T) * Utils::stdNormCdf(-d2) -
					 S0 * Utils::stdNormCdf(-d1);

	EXPECT_NEAR(cosPrice, bsPrice, 1e-5)
		<< "COS should match BS analytical for put";
}
...
TEST_F(COSPricerTest, HestonCFvsKnownPrice)
{
	...
	double cosPrice =
		COSPricer::callPrice(S0, K, r, T, chf, 256, 12.0, std::sqrt(v0));

	// Heston call should be close to BS with vol = sqrt(v0) for short
	// maturities but differ due to stochastic vol. Just check it's reasonable.
	EXPECT_GT(cosPrice, 0.0);
	EXPECT_LT(cosPrice, S0);

	// Put-call parity check
	double putPrice =
		COSPricer::putPrice(S0, K, r, T, chf, 256, 12.0, std::sqrt(v0));
...
TEST_F(StoepComparisonTest, ExternalVolatilitySurfaceFitting)
{
	...
	std::cout
		<< "  (not Heston-generated -> demonstrates arbitrary smile fitting)\n";
	std::cout << "============================================================="
				 "===\n\n";
	...
	for (double maturity : T)
	{
		...
		HestonSLVPathSimulator2D slvSim(heston, &uShapeSurface, times, extPaths,
										extBins, seed);
	...
	std::cout << "Max absolute error: " << maxAbsError << " bp\n";
	std::cout << "(With external surface, SLV adjusts leverage to match "
				 "arbitrary smiles)\n";
	...
	EXPECT_LT(maxAbsError, 250.0)
		<< "SLV should fit external surface within tolerance!";
}
...
// Key: same Brownians for both SLV and Heston MC -> high correlation ->
// variance reduction
...
TEST_F(StoepComparisonTest, ControlVariateVarianceReduction)
{
	...
	std::cout << "Heston analytical (COS): " << std::fixed
			  << std::setprecision(6) << hestonAnalytical << "\n\n";
	...
	std::cout << "  95% CI:    [" << cvResult.ci95Lower << ", "
			  << cvResult.ci95Upper << "]\n";
	...
	std::cout << "  95% CI:    [" << plainResult.ci95Lower << ", "
			  << plainResult.ci95Upper << "]\n";

	double varReduction = plainResult.stdError / cvResult.stdError;
	std::cout << "\nVariance reduction factor: " << std::setprecision(2)
			  << varReduction << "x\n";
...
TEST_F(StoepComparisonTest, ControlVariateShowsVarianceReduction)
{
	...
	for (double K : {0.90, 0.95, 1.0, 1.05, 1.10})
	{
		...
		std::cout << std::fixed << std::setprecision(2) << "  " << K << "    "
				  ...
				  << cvResult.stdError << "    " << std::setprecision(1)
				  << reduction << "x\n";
	}

	avgReduction /= count;
	std::cout << "\nAverage variance reduction: " << std::setprecision(1)
			  << avgReduction << "x\n";
...
TEST_F(StoepComparisonTest, MixingFactorInterpolation)
{
	...
	for (double eta : etas)
	{
		HestonSLVPathSimulator2D simulator(heston, times, nPaths, numBins,
										   seed);
		...
		std::cout << std::fixed << std::setprecision(2) << "  " << eta
				  << "      " << std::setprecision(6) << slvPrice << "   "
				  << std::showpos << std::setprecision(1) << errorVsHeston
				  << " bp" << std::noshowpos << "\n";
	}

	// η=1 (full SLV) should be close to pure Heston when using
	// Heston-consistent local vol
	double eta1Error =
		std::abs(slvPrices[4] - hestonPrice) / hestonPrice * 10000;
	std::cout << "\nη=1 vs pure Heston error: " << std::setprecision(1)
			  << eta1Error << " bp\n";

	// η=1 SLV should be close to pure Heston (within ~100bp MC noise +
	// discretization error)
	EXPECT_LT(eta1Error, 150.0) << "η=1 SLV should match pure Heston when "
								   "using Heston-consistent local vol";
}
