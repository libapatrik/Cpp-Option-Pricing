...
TEST(PDESolverTest, HeatEquation_AnalyticalSolution)
{
    ...
    ExplicitSolver explicitSolver(pde, grid, bc);
    ...
    ImplicitSolver implicitSolver(pde, grid, bc);
    ...
    CrankNicolsonSolver cnSolver(pde, grid, bc);
...
TEST(PDESolverTest, Convergence_HeatEquation)
{
    ...
    for (int refinement = 0; refinement < 4; ++refinement) {
        ...
        CrankNicolsonSolver solver(pde, grid, bc);
        solver.solve();
