...
public:
	OptResult solve(ObjectiveFunc f, GradientFunc grad,
					const std::vector<double> &x0, const GDOptions &opts = {});
};
...
{
public:
	LMResult solve(ResidualFunc residuals, const std::vector<double> &x0,
...
private:
	Matrix numericalJacobian(ResidualFunc f, const std::vector<double> &x,
							 double h = 1e-7);
	std::vector<double> clamp(const std::vector<double> &x,
							  const std::vector<double> &lb,
							  const std::vector<double> &ub);
	void clampInPlace(std::vector<double> &x, const std::vector<double> &lb,
					  const std::vector<double> &ub);
};
