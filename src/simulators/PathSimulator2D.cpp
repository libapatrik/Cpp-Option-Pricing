									   ...
									   double Z_V) const
{
	...
	double s2 = V_plus * (xi * xi / kappa) * (ekt - ekt * ekt) + vbar * (xi * xi / (2.0 * kappa)) * (1.0 - ekt) * (1.0 - ekt);
									   ...
									   double psi_c) const
{
	...
	// Generate ALL randoms upfront to ensure consistent consumption
...
size_t HestonSLVPathSimulator2D::calibrateLeverage(size_t maxIterations, double tol, double damping)
{
	...
	// Keep _leverageCalibrated = false during the iteration loop
